...
import { Star, Chevron, Plane } from "../../components/icons";

export const metadata = {
  title: "Reviews | AeroLux Travel",
  description: "Genuine AeroLux Travel reviews from travelers who trust us for premium flights.",
};
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-navy">
        ...
        <div className="container-x relative z-10 py-10 sm:py-14">
          <nav className="mb-8 text-sm text-white/60">
            <Link href="/" className="hover:text-gold">Home</Link> <span className="px-1">›</span>
            <span className="text-white/90">Reviews</span>
          </nav>
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div className="text-white">
              <h1 className="font-serif text-4xl font-bold sm:text-5xl">AeroLux Travel Reviews</h1>
              <p className="mt-5 max-w-md text-white/70">
                Discover genuine AeroLux Travel reviews and customer testimonials. Read real
                experiences from travelers who trust us for premium flights, and see why we're a
                legitimate choice for luxury travel planning.
              </p>
            </div>
            <div className="rounded-2xl bg-white/[0.06] p-6 ring-1 ring-white/10 sm:p-8">
              <div className="flex justify-center"><Stars /></div>
              <h2 className="mt-3 text-center font-serif text-2xl font-bold text-white">AeroLux Travel Rating 4.8/5</h2>
              <p className="mt-1 text-center text-sm text-white/60">Based on 146,452 reviews. 95% of customers recommend AeroLux Travel</p>
        ...
        <div className="container-x grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-5">
          {sources.map((s) => (
            <div key={s.name} className="text-center">
              ...
              <p className="mt-2 text-sm font-semibold text-navy">{s.name}</p>
              <p className="text-xs text-ink/50">Based on more than {s.count} reviews</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-mist py-16 sm:py-20">
        <div className="container-x">
          <h2 className="section-title">Travel Video Reviews</h2>
          <p className="mt-4 max-w-2xl text-ink/65">
            Explore the world through the eyes of fellow travelers. Immerse yourself in authentic
            firsthand accounts — helpful tips and memorable moments that go beyond traditional reviews.
          </p>
          <div className="mt-10 grid gap-6 sm:grid-cols-3">
            {videos.map((v) => (
              <button key={v.title} className={`group relative h-56 overflow-hidden rounded-2xl bg-gradient-to-br ${v.gradient} text-left`}>
                ...
                <span className="absolute bottom-4 left-4 right-4 font-serif text-lg font-bold text-white">{v.title}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {reviews.map((r) => (
              <figure key={r.name} className="flex flex-col rounded-xl bg-mist p-5">
                ...
                <blockquote className="mt-4 flex-1 text-sm leading-relaxed text-ink/75">{r.text}</blockquote>
