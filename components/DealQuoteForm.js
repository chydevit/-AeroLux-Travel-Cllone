    ...
    return (
      <div className="rounded-2xl bg-white p-8 text-center shadow-lift">
        ...
        <p className="mx-auto mt-2 max-w-xs text-sm text-ink/65">
          A travel manager will call you shortly with the best fares to {city}.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white p-5 shadow-lift sm:p-6">
      <div className="mb-4 rounded-xl bg-mist p-4 text-center">
        <p className="text-sm font-semibold text-navy">Call to Learn More or Book</p>
        <a href="tel:+18555550199" className="font-serif text-2xl font-bold text-navy">855-555-0199</a>
        <p className="mt-1 text-xs text-ink/55">or submit a request below</p>
      </div>

      <div className="mb-3 flex gap-1 rounded-full bg-mist p-1 text-xs font-semibold">
        <button type="button" onClick={() => setRt(true)} className={`flex-1 rounded-full py-1.5 ${rt ? "bg-navy text-white" : "text-ink/60"}`}>Round-Trip</button>
        <button type="button" onClick={() => setRt(false)} className={`flex-1 rounded-full py-1.5 ${!rt ? "bg-navy text-white" : "text-ink/60"}`}>One-Way</button>
      </div>

      <form onSubmit={(e) => { e.preventDefault(); setDone(true); }} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <input className={input} placeholder="From*" required />
          <input className={input} placeholder="To*" defaultValue={city} required />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <input type="date" className={input} aria-label="Depart date" />
          <input type="date" className={input} aria-label="Return date" disabled={!rt} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <input className={input} placeholder="Name*" required />
          <select className={input} aria-label="Cabin" defaultValue={cabins[0]}>
        ...
        <button type="submit" className="btn-gold w-full py-3 text-base">
          <Plane className="h-5 w-5" /> Get Free Quotes
        </button>
