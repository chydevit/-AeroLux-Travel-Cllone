...
import { Phone, Headset, Globe } from "./icons";
...
export default function ContactPageContent() {
  ...
  return (
    <>
      ...
      >
        <a href={contact.phoneLink} className="btn-gold">{t("Call")} {contact.phone}</a>
      </PageHero>
        ...
        <div className="container-x grid gap-10 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            {cards.map((c) => {
              ...
              const inner = (
                <div className="flex gap-4 rounded-2xl border border-navy/10 bg-mist p-6 transition hover:border-gold/40">
                  ...
                  <div>
                    <h3 className="font-serif text-lg font-bold text-navy">{t(c.title)}</h3>
                    {c.lines.map((l) => <p key={l} className="text-sm text-ink/65">{t(l)}</p>)}
                  </div>
                </div>
              );
            ...
            <div className="rounded-2xl bg-gradient-to-br from-navy to-navy-800 p-6 text-white">
              <h3 className="font-serif text-lg font-bold">{t("Prefer a quick quote?")}</h3>
              <p className="mt-1 text-sm text-white/70">{t("Tell us your route and dates and we'll do the rest.")}</p>
              <a href="#quote" className="btn-gold mt-4">{t("Request a Free Quote")}</a>
            </div>
          </div>
