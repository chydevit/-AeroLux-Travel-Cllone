          ...
          {posts.map((p) => (
            <article key={p.title} className="group overflow-hidden rounded-2xl bg-white shadow-card transition hover:-translate-y-1.5 hover:shadow-lift">
              <div className={`relative h-40 bg-gradient-to-br ${p.gradient}`}>
                ...
                <span className="absolute left-3 top-3 rounded-full bg-gold px-2.5 py-1 text-xs font-bold text-navy">{t(p.tag)}</span>
              </div>
              <div className="p-5">
                <h2 className="font-serif text-lg font-bold text-navy">{t(p.title)}</h2>
                <p className="mt-2 text-sm leading-relaxed text-ink/65">{t(p.excerpt)}</p>
                <a href="#quote" className="mt-3 inline-block text-sm font-semibold text-gold hover:underline">{t("Read more")} →</a>
              </div>
            </article>
          ))}
        </div>
