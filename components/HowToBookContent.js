      ...
      <section className="bg-white py-16">
        <div className="container-x">
          <div className="rounded-3xl bg-gradient-to-br from-navy to-navy-800 p-8 text-center text-white sm:p-12">
            <h2 className="font-serif text-3xl font-bold">{t("Ready When You Are")}</h2>
            <p className="mx-auto mt-3 max-w-xl text-white/70">
              {t("Speak with a travel manager 24/7 or request a free quote and we'll get back to you within minutes.")}
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
              <a href="#quote" className="btn-gold">{t("Request a Free Quote")}</a>
              <a href={contact.phoneLink} className="btn-outline-gold">{t("Call")} {contact.phone}</a>
            </div>
          </div>
        </div>
      </section>
