  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto bg-navy-900/70 p-4 backdrop-blur-sm sm:items-center">
      <div className="relative my-auto w-full max-w-lg rounded-2xl bg-white p-6 shadow-lift sm:p-8">
        <button onClick={close} aria-label="Close" className="absolute right-4 top-4 text-ink/40 hover:text-navy">
        ...
        {done ? (
          <div className="py-8 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-sm text-ink/65">
              A dedicated travel manager will reach out shortly with itinerary options
              that match your request. Expect to hear from us within minutes.
            </p>
            <button onClick={close} className="btn-gold mt-6">Close</button>
          </div>
        ) : (
          <>
            <p className="eyebrow mb-1">Free &amp; No Obligation</p>
            <h3 className="font-serif text-2xl font-bold text-navy">Request a Free Quote</h3>
            ...
            <form onSubmit={(e) => { e.preventDefault(); setDone(true); }} className="mt-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <input className={input} placeholder="To*" required />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <input type="date" className={input} aria-label="Departure date" />
              ...
              <p className="text-[11px] leading-relaxed text-ink/45">
                By submitting this form you agree to be contacted about travel options
                by phone, text and email. No purchase necessary. Your privacy is respected.
              </p>
            </form>
          </>
        )}
